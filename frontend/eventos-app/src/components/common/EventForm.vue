<template>
    <div class="event-form-view">
      <v-container fluid class="fill-height event-form-container">
        <v-row justify="center">
          <v-col cols="12" md="8" lg="6">
            <v-card class="elevation-4 event-form-card">
  ...
  export default {
    ...
    setup(props) {
      ...
      const fechaInicioRules = [
        ...
        v => isValidDateTime(v) || 'Formato de fecha inválido (YYYY-MM-DD HH:MM:SS)',
      ];
      const fechaFinRules = [
      ...
      const submitForm = async () => {
        ...
        try {
          ...
          if (isUpdateMode.value) {
            response = await api.put(`/eventos/actualizar/${props.eventId}`, payload);
            successMessage.value = response.data.message || 'Evento actualizado exitosamente';
          } else {
            response = await api.post('/eventos/crear', payload);
            successMessage.value = response.data.message || 'Evento creado exitosamente';
          }
      ...
      const handleError = error => {
        if (error.response) {
          ...
          const messages = {
            ...
            404: data.message || (isUpdateMode.value ? 'Evento no encontrado' : 'Error desconocido'),
            500: 'Error del servidor. Intenta de nuevo más tarde',
          };
          errorMessage.value = messages[status] || `No se pudo ${isUpdateMode.value ? 'actualizar' : 'crear'} el evento`;
          if (status === 401) {
            authStore.logout();
            router.push('/login');
          }
